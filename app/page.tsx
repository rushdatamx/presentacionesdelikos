...
import Slide2RelacionComercial from "./components/charts/Slide2-RelacionComercial"
import Slide3TopProductos from "./components/charts/Slide3-TopProductos"
import Slide4PerformanceTiendas from "./components/charts/Slide4-PerformanceTiendas"
import Slide5Oportunidades from "./components/charts/Slide5-Oportunidades"

const slides = [
  { id: 1, name: "Portada", component: Slide1Portada },
  { id: 2, name: "Relacion Comercial", component: Slide2RelacionComercial },
...
export default function Home() {
  ...
  const exportToPDF = async () => {
    ...
    pdf.save("DELIKOS-MERCO-Revision-Negocio-2025.pdf")
    setExporting(false)
    setCurrentSlide(0)
  }
  ...
  const exportCurrentSlide = async () => {
    if (slideRef.current) {
      try {
        ...
        link.download = `DELIKOS-MERCO-${currentSlide + 1}-${slides[currentSlide].name}.png`
        link.href = imgData
        link.click()
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-[1280px] mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">DELIKOS + MERCO</h1>
          <p className="text-gray-500">Revision de Negocio 2025</p>
        </div>
